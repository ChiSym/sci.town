// from https://github.com/xylk/prosemirror-firebase/blob/master/src/index.js (MIT License)
// modified 2024 for modern firebase api compat
...
module.exports.FirebaseEditor = class FirebaseEditor {
    constructor({
                    ...
                    stateConfig,
                    view: constructView,
                    clientID: selfClientID = push(firebaseRef).key,
                    progress = _ => _
                }) {
        progress(0 / 2)
        const this_ = this
        ...
        const constructEditor = get(checkpointRef).then(
            function (checkpointSnapshot) {
                progress(1 / 2)
                let {d, k: latestKey = -1} = checkpointSnapshot.val() || {}
                ...
                return get(query(changesRef, orderByKey(), startAt(String(latestKey + 1)))).then(
                    function (snapshot) {
                        ...
                        onChildAdded(query(changesRef, orderByKey(), startAt(String(latestKey + 1))), function (snapshot) {
