(ns studio.proxy
  "Proxy websocket messages from producers to studios"
...
(defn upgrade! [{:as req :keys [url]} server]
  ...
  (let [URL (js/URL. url)
        ...
        mode (case (.-pathname URL)
                       ;; studio clients should initialize websocket at /ws-studio
               "/ws-studio" :studio
    ...
    {:maxPayloadLength (* 1024 1024 64)              ;; max message size 64MB
     ...
     :open (fn [{:as ws {:keys [id mode] :as data} :data}]
             (case mode
               ...
               :studio (do
                                         ;; always subscribe studios to the list of gen clients
                         (.subscribe ws :producers)
                                         ;; send initial value of producers to studio
     ...
     :message (fn [{:as ws {:keys [id mode] :as data} :data} message]
                (case mode
                               ...
                               ;; gen clients to subscribed studios.
                  :gen (.publish @!server id message) ;; proxy message without touching it

                  :studio (let [[op & args] (msgpack/decode message)]
                            (case op
